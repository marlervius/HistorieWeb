...
- Lokal framdrift bruker kapittelspesifikk, versjonert nøkkel og tåler korrupte eller utdaterte data.
...
En leveranse er ferdig når innholdsmodellen validerer, interne lenker virker, ingen manglende filer lenkes, og `npm run typecheck`, `npm run lint` og `npm test` er grønne. Layout- eller interaksjonsendringer krever i tillegg reell kontroll i nettleser på mobil og desktop.
