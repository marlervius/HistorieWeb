...
export default async function ChapterPage({ params }: { params: Promise<Params> }) {
  ...
  const understanding = chapter.understandingLabels ?? {
    heading: "Årsaker og virkninger",
    intro: "",
    leftTitle: "Mulige forutsetninger",
    rightTitle: "Mulige følger",
    chainTitle: "En mulig årsakskjede",
    chainIntro: "Hvert ledd må forklares, og kjeden må formuleres som en mulighet. Ingen av pilene betyr at neste ledd alltid fulgte.",
    comparisonTitle: "Brudd og kontinuitet",
    breaksTitle: "Brudd",
    continuitiesTitle: "Kontinuitet",
  };
  return <PageShell>
    <main className="page-main" id="main">
      ...
      <div className="chapter-layout">
        ...
        <article className="chapter-content">
          ...
          <section className="article-section" id="forstaelse"><div className="section-heading"><span className="eyebrow">02 · Forståelse</span><h2>{understanding.heading}</h2></div>{understanding.intro && <p>{understanding.intro}</p>}<div className="columns-2"><div className="content-box"><h3>{understanding.leftTitle}</h3><ul className="plain-list">{chapter.causes.map((cause) => <li key={cause}>{cause}</li>)}</ul></div><div className="content-box ochre"><h3>{understanding.rightTitle}</h3><ul className="plain-list">{chapter.effects.map((effect) => <li key={effect}>{effect}</li>)}</ul></div></div><h3>{understanding.chainTitle}</h3><p>{understanding.chainIntro}</p><div className="chain">{chapter.causeChain.map((step) => <div className="chain-step" key={step}>{step}</div>)}</div><h3>{understanding.comparisonTitle}</h3><div className="comparison"><table><thead><tr><th>{understanding.breaksTitle}</th><th>{understanding.continuitiesTitle}</th></tr></thead><tbody><tr><td><ul className="plain-list">{chapter.breaks.map((item) => <li key={item}>{item}</li>)}</ul></td><td><ul className="plain-list">{chapter.continuities.map((item) => <li key={item}>{item}</li>)}</ul></td></tr></tbody></table></div></section>
          ...
          <section className="article-section" id="lange-linjer"><div className="section-heading"><span className="eyebrow">06 · Lange linjer</span><h2>Kobling til større utviklinger</h2></div><div className="line-grid">{chapter.longLineIds.map((id) => { const line = longLines.find((candidate) => candidate.id === id); return line ? <article className="line-card" key={line.id}><h3>{line.title}</h3><p>{line.description}</p></article> : null; })}</div><div className="content-box ochre"><h3>Skriv med lange linjer</h3><p>{chapter.longLinesPrompt ?? "Velg én linje. Skriv et avsnitt med minst tre konkrete historiske punkter fra kapitlet som linjen går gjennom. Vurder hvilke sammenhenger som er godt dokumentert, og hvor du må bruke forbehold."}</p></div></section>
          ...
          <section className="article-section" id="kilder"><div className="section-heading"><span className="eyebrow">09 · Faglig grunnlag</span><h2>Kilder og videre lesing</h2></div><p>Numrene i faktapunktene, tidslinjen og kildeblikket peker direkte til kildene som støtter påstanden.</p><ul className="source-list">{chapter.sources.map((source, index) => <li id={`kilde-${source.id}`} key={source.id}><strong><span className="source-number">[{index + 1}]</span> <a href={source.href} target="_blank" rel="noreferrer">{source.title}</a></strong><span>{source.note}</span><span><strong>Rettighet/proveniens:</strong> {source.rights}</span></li>)}</ul><p className="local-note">Teksten og oppgavene er bearbeidet for undervisning; de er ikke direkte sitater.</p></section>
